...
void pmm_init(struct multiboot_info *mboot_info){
    ...
    phys_block_count = (phys_installed_memory_size) / PHYS_BLOCK_SIZE;          // how many blocks will
    ...
    memset(phys_memory_bitmap, 0xFF, phys_block_count / PHYS_BLOCKS_PER_BYTE);  // initially mark all installed memory as used
    pmm_free_available_memory(mboot_info);                                      // frees memory that GRUB thinks is available
    pmm_alloc_chunk(KERNEL_START_PADDR, KERNEL_SIZE);                           // from the freed memory, we need to allocate those that are used by the kernel
    
    pmm_alloc_chunk((uint64_t)(intptr_t)phys_memory_bitmap, phys_block_count);  // we also need to allocate the memory used by the physical card itself.
